...
class Applicants(Resource):
    def _get(self, applicant_id: UUID) -> Response:
        return self._session.get(url=f"{self._api_endpoint}/api/v3/applicants/{applicant_id}", timeout=self._timeout)

    @validate_arguments
    def get(self, applicant_id: UUID) -> Applicant:
        response = self._get(applicant_id)
        assert response.status_code == 200, f"get applicant failed, expected 200 got {response.status_code}, body: {response.text}"
        data = response.json()
    ...
    def _list(self, page: int = 0, limit: int = 10) -> Response:
        return self._session.get(url=f"{self._api_endpoint}/api/v3/applicants?page={page}&?limit={limit}",
                                 timeout=self._timeout)

    @validate_arguments
    def list(self, page: int = 0, limit: int = 10) -> PagedResponse[Applicant]:
        response = self._list(page, limit)
        assert response.status_code == 200, f"list applicants failed, expected 200 got {response.status_code}, response {response.text}"
        data = response.json()
        return PagedResponse[Applicant](**data)

    def _delete(self, applicant_id: UUID) -> Response:
        return self._session.delete(url=f"{self._api_endpoint}/api/v3/applicants/{applicant_id}", timeout=self._timeout)

    @validate_arguments
    def delete(self, applicant_id: UUID) -> None:
        response = self._delete(applicant_id)
        assert response.status_code == 200, f"delete applicant failed, expected 200 got {response.status_code}, response {response.text}"
